...
import com.example.foodapp.interfaced.ChangeNumberItemsListener;
...
public class CartListAdaptor extends RecyclerView.Adapter<CartListAdaptor.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CartListAdaptor.ViewHolder holder, int position) {
        ...
        holder.totalEachItem.setText(String.valueOf(
                Math.round((foodDomains.get(position).getNumberInCart() *
                        foodDomains.get(position).getFee()) * 100) / 100));
        holder.num.setText(String.valueOf(foodDomains.get(position).getNumberInCart()));

        int drawableResourceId = holder.itemView.getContext().getResources().getIdentifier(
                foodDomains.get(position)
                        .getPic(),
                "drawable",
